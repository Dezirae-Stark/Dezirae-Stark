...
- **Quality** — 404/404 backend tests passing; Flutter analyze clean on touched files

**Status:** v3.1.0-dev complete: hedge engine (PR #5), live execution layer (PR #6), orchestrator wiring, live trading session, Flutter Live tab, imitation-learning framework (PR #7), v1 state persistence, and UI-driven runtime config overrides — all shipped behind safety defaults. Live execution defaults to disabled; flip the kill switch from the Settings → Runtime Configuration toggle (typed-string confirmation required) only when ready.
