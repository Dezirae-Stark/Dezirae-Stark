...
|:---|:---|
| **Ghost Intel 98 — Win98-Styled Investigative Case-Management Suite** *(formerly Dead Cyber Society 98 / Ghost Access 98)* | Offline-first Win98-styled Electron/React/TS investigative case-management + OSINT desktop: GeoINT 3D threat map, EyeSpy CCTV wall, encrypt-at-rest cases, and an offline AI assistant (voice, RAG, neural TTS). Its opt-in Tor-only PQ-hybrid (X25519 + ML-KEM-1024) chat is **formally verified internally** (ProVerif symbolic + CryptoVerif computational) — external audit + FIPS build the only open gates. **1,979 automated tests; v3.22.3 latest** (X / Twitter collection now works on Windows — the per-OS `twscrape` sidecar `.exe` is built on a real Win11 VM, SHA-pinned, and bundled; plus the v3.22.2 Searchlight **Connect-Tor** button + launchable **X / Twitter** window — riding on the v3.22.0 SOCMINT activation: live **Telegram / WhatsApp / X** collectors, egress off by default, **gate-before-egress** + **`socks5h`** no-DNS-leak fail-closed transport adversarially verified, Baileys supply-chain-pinned against the `lotusbail` clone). *Full v3.2→v3.22.3 history in the project card below.* |
