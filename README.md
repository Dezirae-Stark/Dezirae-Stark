...
| **OSINT tooling** | Ghost Intel 98 (offline case-management desktop) + GhostExodus counter-extremism suite | 2,072 tests / v1.1.0 |
...
|:---|:---|
| **Ghost Intel 98 — Win98-Styled Investigative Case-Management Suite** *(formerly Dead Cyber Society 98 / Ghost Access 98)* | Offline-first Win98-styled Electron/React/TS investigative case-management + OSINT desktop: GeoINT 3D threat map, EyeSpy CCTV wall, encrypt-at-rest cases, and an offline AI assistant (voice, RAG, neural TTS). Its opt-in Tor-only PQ-hybrid (X25519 + ML-KEM-1024) chat is **formally verified internally** (ProVerif symbolic + CryptoVerif computational) — external audit + FIPS build the only open gates. **2,072 automated tests; v3.23.0 latest** (Searchlight **detection scorer** kills soft-404 false positives — an **adaptive two-phase probe** scores ~25 structural signals (`og:type`, JSON-LD `Person`, title/canonical, profile-vs-error DOM) so a styled "account doesn't exist" 200 reads as an error, not a confident FOUND; a first-class **MAYBE** tier, sortable columns, live progress + summary panel, and a fail-safe **site-DB override** button; a ported MIT [Aliens_eye](https://github.com/arxhr007/Aliens_eye) logistic-regression **ML model bundled but off by default** pending a fingerprint-free retrain — the heuristic fix ships independently; built 15-task subagent-driven + adversarial whole-branch review, four findings fixed before merge). *Full v3.2→v3.23.0 history in the project card below.* |
