...
- **AI assistant** — pluggable local (Ollama, one-click wizard) / remote LLM, opt-in case context, API keys encrypted, **offline on-device text-to-speech** (cloud voices refused) + a stop control
...
**Security:** offline-first, no telemetry, consent-gated egress. Encrypt-at-rest reviewed across three adversarial rounds; the 3.2.x surface and every v3.3.0 addition went through **multiple dedicated red-team rounds** (0 Critical; all High/Medium fixed + regression-tested — TTS no-cloud *enforced*, media streaming path-confined + lock-revoked, Firefox launcher injection-proofed, Bookmarks import/favicon hardened).

**Release:** [v3.3.0 — Windows installer + full source](https://github.com/Dezirae-Stark/ghost-access-98/releases/tag/v3.3.0)
