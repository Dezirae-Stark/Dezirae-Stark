...
| **OSINT tooling** | Ghost Intel 98 (offline case-management desktop) + GhostExodus counter-extremism suite | 1,393 tests / v1.1.0 |
...
|:---|:---|
| **Ghost Intel 98 — Win98-Styled Investigative Case-Management Suite** *(formerly Dead Cyber Society 98 / Ghost Access 98)* | Offline-first Win98-styled Electron/React/TS investigative case-management + OSINT desktop: GeoINT 3D threat map, EyeSpy CCTV wall, encrypt-at-rest cases, and an offline AI assistant (voice, RAG, neural TTS). Its opt-in Tor-only PQ-hybrid (X25519 + ML-KEM-1024) chat is **formally verified internally** (ProVerif symbolic + CryptoVerif computational) — external audit + FIPS build the only open gates. **1,393 automated tests; v3.21.0 latest** (view CCTV streams over Tor via a `ga98cctv://` main-side Tor media proxy; plus Searchlight readability + a dependency-free PDF report export, a Settings → GeoINT pane surfacing the encrypted AIS key, and ADS-B retry-with-backoff). *Full v3.2→v3.21.0 history in the project card below.* |
